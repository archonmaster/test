...
class Api extends \Cockpit\Controller {
    ...
    public function updateentry() {
        ...
        if ($collection && $entry) {
            ...
            $col = "collection".$collection["_id"];            
            if (!isset($entry["_id"])) return '{"success":false}';
          
          	$src_entry = $this->app->db->findOne("collections/{$col}", ["_id" => $entry["_id"]]);
            if (!isset($src_entry)) return '{"success":false}';
          
            $src_entry["modified"] = time();
            //$entry["_uid"]     = @$this->user["_id"];
            if (isset($src_entry["user_id"])) {
                $src_entry["_uid"] = $src_entry["user_id"];
                unset($src_entry["user_id"]);
            }

...
class Mediamanager extends \Cockpit\Controller {
        ...
        if (isset($files['name']) && $path && file_exists($targetpath)) {
            for ($i = 0; $i < count($files['name']); $i++) {
                $result = false;
              	...
              	if (!$files['error'][$i]) {
                    // Check if image
                    $type = exif_imagetype($files['tmp_name'][$i]);
                    $allowedTypes = [
                        1,  // [] gif
                        2,  // [] jpg
                        3,  // [] png
                  	...
                  	$convert2jpg = $this->param('convert2jpg', false);
                  	if ($convert2jpg == "false") $convert2jpg = false;
                    $maxx = $this->param('maxWidth', false);
                    $maxy = $this->param('maxHeight', false);
                    if ($maxx == "auto") $maxx = false;
                    if ($maxy == "auto") $maxy = false;
                    
                  	$clean = [];
                  	$pinfo = pathinfo($files['name'][$i]);
                    if ($uniq) {
                      $clean["name"] = uniqid();
                      $clean["ext"] = strtolower($pinfo["extension"]);
                    } else {
                      $clean["name"] = preg_replace('/[^a-zA-Z0-9-_\.]/','', str_replace(' ', '-', $files['name'][$i]));
                      $clean["name"] = basename($clean["name"],'.'.$pinfo["extension"]);
                      $clean["ext"] = $pinfo["extension"];
                    }

                    // if not image
                    if (!in_array($type, $allowedTypes)) { 
                      	$result = move_uploaded_file($files['tmp_name'][$i], "{$targetpath}/{$clean["name"]}.{$clean["ext"]}");
                    } else {
                        switch ($type) { 
                            case 1 : 
                                $im = imageCreateFromGif($files['tmp_name'][$i]); 
                            break; 
                            case 2 : 
                                $im = imageCreateFromJpeg($files['tmp_name'][$i]); 
                            break; 
                            case 3 : 
                                $im = imageCreateFromPng($files['tmp_name'][$i]); 
                            break; 
                            case 6 : 
                                $im = imageCreateFromBmp($files['tmp_name'][$i]); 
                            break; 
                        }
                        
                        if (!$maxx && !$maxy) {
                          	if ($convert2jpg) {
                          		$result = imagejpeg($im, "{$targetpath}/{$clean["name"]}.jpg");
                            } else {
                              	$result = move_uploaded_file($files['tmp_name'][$i], "{$targetpath}/{$clean["name"]}.{$clean["ext"]}");
                            }
                          	imagedestroy($im);
                            ...
                            if (!$maxx && $maxy) {
                                if ($height > $maxy) {
                                    $width = round($width * ($maxy / $height));
                                    $height = $maxy;
                                }
                            } elseif ($maxx && !$maxy) {
                                if ($width > $maxx) {
                                    $height = round($height * ($maxx / $width));
                                    $width = $maxx;
                                }
                            } else {
                                if ($width > $maxx) {
                            ...
                            if ($width != $size[0] || $height != $size[1]) {
                                ...
                                imagecopyresampled($image, $im, 0, 0, 0, 0, $width, $height, $size[0], $size[1]);
                                imagedestroy($im);
                                $result = imagejpeg($image, "{$targetpath}/{$clean["name"]}.jpg");
                                imagedestroy($image);
                            } else {
                                if ($convert2jpg) {
                                    $result = imagejpeg($im, "{$targetpath}/{$clean["name"]}.jpg");
                ...
                if ($result) {
                  	if ($convert2jpg) {
                  		$uploaded[] = "{$clean["name"]}.jpg";
                    } else {
                      	$uploaded[] = "{$clean["name"]}.{$clean["ext"]}";
                    }
                } else {
                    $failed[] = $files['name'][$i];
                }
    ...
    protected function getfilelist() {
        ...
        foreach (new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($this->root)) as $file) {
          	...
          	if ($file->getFilename() == "..") continue; // hack for open_basedir restriction error in root folder

(function($){
    ...
    App.module.controller("region", function($scope, $rootScope, $http, $timeout, Contentfields, $photopicker){
                ...
                if (data && Object.keys(data).length) {
                    $scope.region._id = data._id;
                  	...
                  	for (var i in data.fields) {
                      	if (data.fields[i].type == "photopicker") {
                          	var field_name = data.fields[i].name;
                          	files[field_name] = angular.copy(data.fields[i].value);
                        }
                    }
                  	...
                  	if (Object.keys(files).length > 0) {
                        ...
                        for (var i in data.fields) {
                            if (data.fields[i].type == "photopicker") {                              	
                                var field_name = data.fields[i].name;
                              	data.fields[i].value = angular.copy(files[field_name]);
                            }
                        }
                      	$http.post(App.route("/api/regions/save"), {"region": data, "createversion": true}).success(function(response){

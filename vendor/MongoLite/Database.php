...
class Database {
    ...
    public function registerCriteriaFunction($criteria) {
        ...
        if (is_array($criteria)) {
            ...
            //var_dump('return '.UtilArrayQuery::buildCondition($criteria).';');
...
class UtilArrayQuery {
    ...
    public static function buildCondition($criteria, $concat = " && ") {
        ...
        foreach ($criteria as $key => $value) {
            ...
            switch($key) {
                ...
                case '!$isset':
                    if (!empty($value)) {
                        foreach ($value as &$val) {
                            $val = \MongoLite\UtilArrayQuery::doc_name($val);
                            $val = "!isset({$val})";
                        }
                        $fn[] = "(".implode(" && ", $value).")";
                    }
                ...
                default:
                    ...
                    $d = \MongoLite\UtilArrayQuery::doc_name($key);
        ...
        //var_dump(trim(implode($concat, $fn)));
    ...
    private static function doc_name($key) {
        $d = '$document';

        ...
        var fields = {
            ...
            'text': {
                ...
                template: function(model, settings) {
                    var disabled = "";
                    if (typeof (settings) != "undefined" && typeof (settings.disabled) != "undefined") {
                        if (settings.disabled == true) disabled = "disabled";
                    }
                    return '<input class="uk-width-1-1 uk-form-large" type="text"  ng-model="'+model+'" ' + disabled + '>';
                }
            },    
            'textarea': {
                label: 'Text Area',
                template: function(model, settings) {
                    ...
                    return '<textarea class="uk-width-1-1 uk-form-large" type="text"  ng-model="'+model+'" ' + disabled + '></textarea>';
                }
            },            
        ...
        return {
            ...
            link: function(scope, elm, attrs, ngModel) {
                ...
                var defer = function() {
                    ...
                    var options = $.extend({type: 'text', classes: attrs.class || false}, JSON.parse(attrs.options || '{}'));
                    var name = attrs.name || false;

                    if (Contentfields.exists(options.type)) {

                        var field = Contentfields.get(options.type), content;

                        content = field.template(attrs.ngModel, options, name);
